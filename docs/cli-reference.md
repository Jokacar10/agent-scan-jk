...
Load argument values from a YAML file instead of passing them all on the command line. Applies to `scan`, `inspect`, and `evo` (not `guard`). **Any** flag from the sections above — global options, MCP server options, and `scan`-only options — is settable.
